@extends('index')
@section('main')
<main class="mg-section">
    <div class="wrap-container">
        <div class="ladi-title">
            <h2>iPhone 14 Pro Max 256GB | Chính hãng VN/A</h2>
        </div>
        <div class="box-detail-product">
            <div class="box-detail-product__left">
                <div class="product-detail grid grid-tempalte-colum-2 align-items-center" style="grid-template-columns: 40% auto;">
                    <div class="ladi-left">
                        <div class="ladi-slider chuyen_banner_moitruong">
                            <div class="ladi-slider__item">
                                <img src="https://cdn2.cellphones.com.vn/x358,webp,q100/media/catalog/product/t/_/t_m_18.png" alt="">
                            </div>
                            <div class="ladi-slider__item"><img src="https://cdn2.cellphones.com.vn/x358,webp,q100/media/catalog/product/t/_/t_m_18.png" alt=""></div>
                            <div class="ladi-slider__item"><img src="https://cdn2.cellphones.com.vn/x358,webp,q100/media/catalog/product/t/_/t_m_18.png" alt=""></div>
                        </div>
                        <div class="ladi-slider">

                        </div>
                    </div>
                    <div class="ladi-right">
                        <div class="product-price mg-section">
                            <span class="old">29.690.000 ₫</span>
                            <span class="sale">29.690.000 ₫</span>
                        </div>
                        <div class="product-type  mg-section">
                            <ul class="flex flex-wrap align-items-center">
                                <li>
                                    <a href="#" class="active">
                                        <div class="product-type__text">1TB</div>
                                        <div class="product-type__price">41.790.000 đ</div>
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <div class="product-type__text">1TB</div>
                                        <div class="product-type__price">41.790.000 đ</div>
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <div class="product-type__text">1TB</div>
                                        <div class="product-type__price">41.790.000 đ</div>
                                    </a>
                                </li>
                                <li>
                                    <a href="#">
                                        <div class="product-type__text">1TB</div>
                                        <div class="product-type__price">41.790.000 đ</div>
                                    </a>
                                </li>
                            </ul>
                        </div>
                        <div class="product-color  mg-section">
                            <div class="product-color__title">Chọn màu để xem giá và chi nhánh có hàng</div>
                            <div class="product-color__list">
                                <ul class="flex gap-10 flex-wrap align-items-center">
                                    <li>
                                        <button class="btn flex align-items-center flex-wrap active">
                                            <span class="color-images"><img src="https://cdn2.cellphones.com.vn/50x50,webp,q100/media/catalog/product/v/a/vang-iphone-14-pro_5.png" alt=""></span>
                                            <div class="product-color__content ">
                                                <div class="product-color__text">
                                                    Vàng
                                                </div>

                                                <div class="product-color__price">
                                                    29.690.000 đ
                                                </div>

                                            </div>
                                        </button>
                                    </li>

                                    <li>
                                        <button class="btn flex align-items-center flex-wrap ">
                                            <span class="color-images"><img src="https://cdn2.cellphones.com.vn/50x50,webp,q100/media/catalog/product/v/a/vang-iphone-14-pro_5.png" alt=""></span>
                                            <div class="product-color__content ">
                                                <div class="product-color__text">
                                                    Vàng
                                                </div>

                                                <div class="product-color__price">
                                                    29.690.000 đ
                                                </div>

                                            </div>
                                        </button>
                                    </li>

                                    <li>
                                        <button class="btn flex align-items-center flex-wrap ">
                                            <span class="color-images"><img src="https://cdn2.cellphones.com.vn/50x50,webp,q100/media/catalog/product/v/a/vang-iphone-14-pro_5.png" alt=""></span>
                                            <div class="product-color__content ">
                                                <div class="product-color__text">
                                                    Vàng
                                                </div>

                                                <div class="product-color__price">
                                                    29.690.000 đ
                                                </div>

                                            </div>
                                        </button>
                                    </li>


                                </ul>
                            </div>
                        </div>

                        <div class="btn btn-cart">
                            <a href="#"><span>Mua ngay</span>   <br> (Giao hanh từ 2 giờ hoặc nhận tại cửa hàng)</a>

                        </div>
                    </div>
                </div>

            </div>
            <div class="box-detail-product__right">
                <div class="box-on-stock-stores">
                    <div class="box-on-stock-options">
                        <div class="box-stock-option" id="box-stock-location-province">
                            Hồ chí minh
                        </div>
                        <div>
                            <select name="" id="">
                                <option value="0">Quận/Huyện</option>
                                <option value="1">Quận 1</option>
                                <option value="1">Quận 2</option>
                                <option value="1">Quận 3</option>
                                <option value="1">Quận 4</option>
                            </select>
                        </div>
                    </div>
                    <div class="box-stock-count mg-section">Còn 1 cửa hàng có sản phẩm</div>
                    <div class="box-on-stock-address">
                        <div class="box-stock-address__item active">
                            <div class="iconl-phone"><span><i class="fas fa-phone-alt"></i></span></div>
                            <div class="tel"><a href="#">02871000912</a></div> -
                            <div class="address"><a href="#">912 - 915 Trần Hưng Đạo, P. Bình Khánh, TP. Long Xuyên, An Giang</a></div>
                        </div>
                        <div class="box-stock-address__item">
                            <div class="iconl-phone"><span><i class="fas fa-phone-alt"></i></span></div>
                            <div class="tel"><a href="#">02871000912</a></div> -
                            <div class="address"><a href="#">912 - 915 Trần Hưng Đạo, P. Bình Khánh, TP. Long Xuyên, An Giang</a></div>
                        </div>
                    </div>
                    <div class="box-warranty-info mg-section">
                        <div class="warranty-title">
                            <p>Thông tin sản phẩm</p>
                        </div>
                        <div class="wraranty-list">
                            <ul>
                                <li>
                                    <div class="iconl"><span><i class="fas fa-mobile"></i></span></div>
                                    <div class="description">Mới, đầy đủ phụ kiện từ nhà sản xuất</div>
                                </li>
                                <li>
                                    <div class="iconl"><span><i class="fas fa-gift"></i></span></div>
                                    <div class="description">Mới, đầy đủ phụ kiện từ nhà sản xuất</div>
                                </li>
                                <li>
                                    <div class="iconl"><span><i class="fas fa-shield-alt"></i></span></div>
                                    <div class="description">Bảo hành 12 tháng tại trung tâm bảo hành Chính hãng. 1 đổi 1 trong 30 ngày nếu có lỗi phần cứng từ nhà sản xuất. (xem chi tiết)</div>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>

            </div>
        </div>


        <div class="block-same-product">
            <div class="same-product-options">
                <ul>
                    <li class="a">
                        <button data-id="pk">Sản phẩm tương tự</button>

                    </li>
                </ul>
            </div>
            <div class="list-same-product" id="pk">
                <div class="same-product__item">
                    <div class="sub-the-same-product">
                        <section class="ladi-section mg-section">
                            <div class="ladi-list chuyen_banner_text">
                                <div class="ladi-item">
                                    <div class="ladi-images">
                                        <img src="https://cdn2.cellphones.com.vn/358x358,webp,q100/media/catalog/product/g/t/gtt_7766_3__1.jpg" alt="">
                                    </div>
                                    <div class="ladi-name">
                                        Xiaomi Redmi Note 12
                                    </div>
                                    <div class="ladi-body ">

                                        <div class="ladi-price flex align-items-center">
                                            <span class="old">
                                                4.490.000 &nbsp;₫
                                            </span>
                                            <div class="new">
                                                4.490.000 &nbsp;₫
                                            </div>
                                        </div>

                                        <div class="ladi-text-sale mg-section">
                                            Trợ giá lên tới 500.000đ khi tham gia thu cũ đổi mới - Giá thu tốt nhất thị trường
                                        </div>
                                        <div class="ladi-rate">
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                        </div>

                                        <div class="ladi-reaction">
                                            <div class="favorite">
                                                <span class="text">Yêu thích</span> <span class="reaction-iconls love"><i class="far fa-heart"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="ladi-precent">
                                        Giảm 24%
                                    </div>
                                </div>
                                <div class="ladi-item">
                                    <div class="ladi-images">
                                        <img src="https://cdn2.cellphones.com.vn/358x358,webp,q100/media/catalog/product/g/t/gtt_7766_3__1.jpg" alt="">
                                    </div>
                                    <div class="ladi-name">
                                        Xiaomi Redmi Note 12
                                    </div>
                                    <div class="ladi-body">

                                        <div class="ladi-price flex">
                                            <span class="old">
                                                4.490.000 &nbsp;₫
                                            </span>
                                            <div class="new">
                                                4.490.000 &nbsp;₫
                                            </div>
                                        </div>

                                        <div class="ladi-text-sale">
                                            Trợ giá lên tới 500.000đ khi tham gia thu cũ đổi mới - Giá thu tốt nhất thị trường
                                        </div>
                                        <div class="ladi-rate">
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                        </div>

                                        <div class="ladi-reaction">
                                            <div class="favorite">
                                                <span class="text">Yêu thích</span> <span class="reaction-iconls love"><i class="far fa-heart"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="ladi-item">
                                    <div class="ladi-images">
                                        <img src="https://cdn2.cellphones.com.vn/358x358,webp,q100/media/catalog/product/g/t/gtt_7766_3__1.jpg" alt="">
                                    </div>
                                    <div class="ladi-name">
                                        Xiaomi Redmi Note 12
                                    </div>
                                    <div class="ladi-body">

                                        <div class="ladi-price flex">
                                            <span class="old">
                                                4.490.000 &nbsp;₫
                                            </span>
                                            <div class="new">
                                                4.490.000 &nbsp;₫
                                            </div>
                                        </div>

                                        <div class="ladi-text-sale">
                                            Trợ giá lên tới 500.000đ khi tham gia thu cũ đổi mới - Giá thu tốt nhất thị trường
                                        </div>
                                        <div class="ladi-rate">
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                        </div>

                                        <div class="ladi-reaction">
                                            <div class="favorite">
                                                <span class="text">Yêu thích</span> <span class="reaction-iconls love"><i class="far fa-heart"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="ladi-item">
                                    <div class="ladi-images">
                                        <img src="https://cdn2.cellphones.com.vn/358x358,webp,q100/media/catalog/product/g/t/gtt_7766_3__1.jpg" alt="">
                                    </div>
                                    <div class="ladi-name">
                                        Xiaomi Redmi Note 12
                                    </div>
                                    <div class="ladi-body">

                                        <div class="ladi-price flex">
                                            <span class="old">
                                                4.490.000 &nbsp;₫
                                            </span>
                                            <div class="new">
                                                4.490.000 &nbsp;₫
                                            </div>
                                        </div>

                                        <div class="ladi-text-sale">
                                            Trợ giá lên tới 500.000đ khi tham gia thu cũ đổi mới - Giá thu tốt nhất thị trường
                                        </div>
                                        <div class="ladi-rate">
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                        </div>

                                        <div class="ladi-reaction">
                                            <div class="favorite">
                                                <span class="text">Yêu thích</span> <span class="reaction-iconls love"><i class="far fa-heart"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="ladi-item">
                                    <div class="ladi-images">
                                        <img src="https://cdn2.cellphones.com.vn/358x358,webp,q100/media/catalog/product/g/t/gtt_7766_3__1.jpg" alt="">
                                    </div>
                                    <div class="ladi-name">
                                        Xiaomi Redmi Note 12
                                    </div>
                                    <div class="ladi-body">

                                        <div class="ladi-price flex">
                                            <span class="old">
                                                4.490.000 &nbsp;₫
                                            </span>
                                            <div class="new">
                                                4.490.000 &nbsp;₫
                                            </div>
                                        </div>

                                        <div class="ladi-text-sale">
                                            Trợ giá lên tới 500.000đ khi tham gia thu cũ đổi mới - Giá thu tốt nhất thị trường
                                        </div>
                                        <div class="ladi-rate">
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                        </div>

                                        <div class="ladi-reaction">
                                            <div class="favorite">
                                                <span class="text">Yêu thích</span> <span class="reaction-iconls love"><i class="far fa-heart"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="ladi-item">
                                    <div class="ladi-images">
                                        <img src="https://cdn2.cellphones.com.vn/358x358,webp,q100/media/catalog/product/g/t/gtt_7766_3__1.jpg" alt="">
                                    </div>
                                    <div class="ladi-name">
                                        Xiaomi Redmi Note 12
                                    </div>
                                    <div class="ladi-body">

                                        <div class="ladi-price flex">
                                            <span class="old">
                                                4.490.000 &nbsp;₫
                                            </span>
                                            <div class="new">
                                                4.490.000 &nbsp;₫
                                            </div>
                                        </div>

                                        <div class="ladi-text-sale">
                                            Trợ giá lên tới 500.000đ khi tham gia thu cũ đổi mới - Giá thu tốt nhất thị trường
                                        </div>
                                        <div class="ladi-rate">
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                            <span><i class="fas fa-star"></i></span>
                                        </div>

                                        <div class="ladi-reaction">
                                            <div class="favorite">
                                                <span class="text">Yêu thích</span> <span class="reaction-iconls love"><i class="far fa-heart"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </section>
                    </div>
                </div>
            </div>

        </div>

        <div class="block-content-product">
            <div class="block-content-product-left">
                <div class="block-description">
                    <div class="block-description__title">Mô tả sản phẩm</div>
                    <div class="block-description__content">
                        Mô tả ở đây
                    </div>
                </div>
                <div class="block-comment">
                    <div class="box-comment">
                        <div class="block-title">
                            <h3>Hỏi và đáp</h3>
                        </div>
                        <div class="box-comment__ask">
                            <form action="">
                                <div class="flex">
                                    <textarea name="" id="" cols="15" rows="8"></textarea>
                                    <button type="submit" class="btn-submit">Gửi</button>
                                </div>
                            </form>
                        </div>

                        <div class="box-comment__cus mg-section">
                            <ul>
                                <li>
                                    <div class="box-comment__infor">
                                        <div class="avatar">V</div>
                                        <div class="name">Võ Thị Xuân Uyên</div>
                                        <div class="comment-time">1 ngày trước</div>
                                    </div>
                                    <div class="box-comment__content mg-section">
                                        Iphone 12 64gb màu xanh chi nhánh nào có hàng vậy ạ?
                                        <div class="btn-question">
                                            Trả lời
                                        </div>
                                    </div>
                                    <div class="box-question">
                                        <div class="box-comment__infor">
                                            <div class="avatar">
                                                <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><rect width="40" height="40" fill="url(#pattern111)"></rect> <defs><pattern id="pattern111" patternContentUnits="objectBoundingBox" width="1" height="1"><use xlink:href="#image0_119_16" transform="scale(0.00333333)"></use></pattern> <image id="image0_119_16" width="300" height="300" xlink:href="data:image/png;base64,"></image></defs></svg>
                                            </div>
                                            <div class="name">Quản Trị Viên</div>
                                        </div>
                                        <div class="box-comment__content mg-section">
                                            Iphone 12 64gb màu xanh chi nhánh nào có hàng vậy ạ?
                                            <div class="btn-question">
                                                Trả lời
                                            </div>
                                        </div>
                                        <div class="box-comment__ask">
                                            <form action="">
                                                <div class="flex">
                                                    <textarea name="" id="" cols="15" rows="8"></textarea>
                                                    <button type="submit" class="btn-submit">Gửi</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>

                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
            <div class="block-content-product-right">
                <div class="block-technical-infor">
                    <div class="technical-title">Thông số kỹ thuật</div>
                    <ul>
                        <li>
                            <p>Kích thước màn hình</p>
                            <p>6.8 inches</p>
                        </li>
                        <li>
                            <p>Kích thước màn hình</p>
                            <p>6.8 inches</p>
                        </li>
                        <li>
                            <p>Kích thước màn hình</p>
                            <p>6.8 inches</p>
                        </li>
                        <li>
                            <p>Camera sau</p>
                            <p>Siêu rộng: 12MP F2.2 (Dual Pixel AF)
                                Chính: 200MP F1.7 OIS ±3° (Super Quad Pixel AF)
                                Tele 1: 10MP F4.9 (10X, Dual Pixel AF) OIS,
                                Tele 2: 10MP F2.4 (3X, Dual Pixel AF) OIS Thu phóng chuẩn không gian 100X</p>
                        </li>
                        <li>
                            <p>Camera sau</p>
                            <p>Siêu rộng: 12MP F2.2 (Dual Pixel AF)
                                Chính: 200MP F1.7 OIS ±3° (Super Quad Pixel AF)
                                Tele 1: 10MP F4.9 (10X, Dual Pixel AF) OIS,
                                Tele 2: 10MP F2.4 (3X, Dual Pixel AF) OIS Thu phóng chuẩn không gian 100X</p>
                        </li>
                    </ul>
                </div>
                <div class="block-new mg-section">
                    <section class="ladi-section">

                            <div class="ladi-aside">
                                <div class="ladi-post">
                                    <div class="ladi-title">
                                        <span><i class="fa-regular fa-newspaper"></i></span>
                                        Tin tức về sản phẩm
                                    </div>
                                    <div class="list-post">
                                        <div class="post-item">
                                            <a href="#">
                                                <div class="post-image">
                                                    <img src="https://cellphones.com.vn/sforum/wp-content/uploads/2023/04/bang-b2-lai-xe-gi.jpg" alt="">
                                                </div>
                                                <div class="post-body">
                                                    <div class="post-content">
                                                        Bằng B2 lái xe gì? Hồ sơ thi bằng lái xe B2 mới nhất
                                                    </div>
                                                </div>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="ladi-btn-loader mg-section">
                                        <div class="action-loading">Xem thêm XXX sản phẩm</div>
                                    </div>
                                </div>
                            </div>

                    </section>
                </div>
            </div>

        </div>

    </div>

</main>

@endsection
